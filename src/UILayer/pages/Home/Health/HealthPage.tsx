...
import { useDailyMetricsQuery, useAddHydrationMutation } from '../../../../DataLayer/APILayer/userQueries';
import { syncHealthDailyMetricsToSupabase } from '../../../../DataLayer/APILayer/supabase/healthMetricsSync';

const HealthPage = () => {
    const { data: metrics, isPending, isError, error, refetch } = useDailyMetricsQuery();
    const hydrationMutation = useAddHydrationMutation();

    useEffect(() => {
        if (!metrics) return;
        void syncHealthDailyMetricsToSupabase(metrics);
    }, [metrics]);
    ...
    const stats = [
        {
            ...
            bg: "bg-green-500/20",
            percentage: metrics?.steps ? (metrics.steps / 10000) * 100 : 0
        },
        {
            label: "Heart Rate",
            value: metrics?.heartRateAvg ? `${metrics.heartRateAvg} bpm` : "-- bpm",
            goal: "RHR: 58",
            icon: <Heart size={20} />,
            color: "text-red-400",
            bg: "bg-red-500/20",
            percentage: 100 // Average is just a display
        },
        {
            label: "Calories",
            value: metrics?.caloriesConsumed?.toLocaleString() || "0",
            goal: "2,200",
            icon: <Flame size={20} />,
            color: "text-orange-400",
            bg: "bg-orange-500/20",
            percentage: metrics?.caloriesConsumed ? (metrics.caloriesConsumed / 2200) * 100 : 0
        },
        {
            label: "Sleep",
            value: metrics?.sleepHours ? `${Math.floor(metrics.sleepHours)}h ${Math.round((metrics.sleepHours % 1) * 60)}m` : "0h 0m",
            goal: "8h 00m",
            icon: <Moon size={20} />,
            color: "text-indigo-400",
            bg: "bg-indigo-500/20",
            percentage: metrics?.sleepHours ? (metrics.sleepHours / 8) * 100 : 0
        },
    ];
    ...
    if (isError) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center gap-4 p-8 text-center">
                <Typography variant="h4" className="text-white font-bold" {...commonProps}>
                    Could not load vitals
                </Typography>
                <Typography className="text-white/60 max-w-md" {...commonProps}>
                    {error instanceof Error ? error.message : 'Request failed. Check that you are signed in and the backend is reachable.'}
                </Typography>
                <button
                    type="button"
    ...
    if (isPending) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center gap-3">
                <Typography variant="h4" className="text-white/20 animate-pulse" {...commonProps}>
                    Synchronizing Vitals...
                </Typography>
                <Typography className="text-white/30 text-sm max-w-sm text-center" {...commonProps}>
                    Loading daily metrics from your ICE GATE backend…
                </Typography>
            </div>
        );
    }
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-white font-medium">
                            ...
                            <span>Updated {metrics?.updatedAt ? new Date(metrics.updatedAt).toLocaleTimeString() : 'just now'}</span>
                        </div>
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2">
                    <GlassCard className="p-6 min-h-[400px]">
                        ...
                        {/* Fake Chart - Still placeholder until weekly API is ready */}
                    ...
                    <GlassCard className="p-6 relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="flex items-end gap-2 mb-2">
                                <Typography variant="h3" className="text-white font-bold" {...commonProps}>{metrics?.waterMl?.toLocaleString() || "0"}</Typography>
                            ...
                            <div className="h-3 bg-white/10 rounded-full overflow-hidden">
                                <motion.div
                                    ...
                                    initial={{ width: 0 }}
                                    animate={{ width: `${Math.min((metrics?.waterMl || 0) / 2500 * 100, 100)}%` }}
                                    transition={{ duration: 1 }}
                                />
                            </div>
                            <div className="mt-6 flex justify-center">
                                <button
                                    onClick={() => hydrationMutation.mutate(250)}
                                    disabled={hydrationMutation.isPending}
                                    className="px-6 py-2 bg-blue-500 hover:bg-blue-400 text-white font-bold rounded-xl shadow-lg shadow-blue-500/20 transition-all active:scale-95 flex items-center gap-2 disabled:opacity-50"
                                >
                                    <Droplet size={16} fill="currentColor" />
                                    {hydrationMutation.isPending ? 'Adding...' : 'Add 250ml'}
                                </button>
                            </div>
                        </div>
                    </GlassCard>
