...
const ChatBox: React.FC = () => {
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50">
            ...
            <div className="border-b border-black font-bold bg-indigo-700! text-white! sticky top-0 backdrop-blur-md p-2 z-10">
                Chat Box
            </div>
            <LiquidGlassCard
                // ref={scrollRef}
                ...
                <div className="p-4 text-sm text-gray-800">
                    {[...chatBoxHistoryStore.chatHistory].reverse().map((chat) => (
                        <div className="mb-6 flex flex-col gap-2" key={chat.index}>
                            {chat.question && (
                                <div className="self-end bg-indigo-100 p-2 rounded-lg max-w-[90%] text-indigo-900 border border-indigo-200">
                                    <div className="font-bold text-[10px] uppercase opacity-50 mb-1">You</div>
                                    <div>{chat.question}</div>
                                </div>
                            )}
                            <div className="self-start bg-white/50 p-3 rounded-lg max-w-[90%] border border-gray-200 shadow-sm backdrop-blur-sm">
            ...
            {/* Input Area */}
            <div className="w-100 bg-indigo-950 rounded-b-lg shadow-lg p-2">
                <input
                    type="text"
                    value={message}
            ...
            <style>{`
                ...
                .markdown-content code { background: rgba(0,0,0,0.05); padding: 0.1rem 0.3rem; border-radius: 4px; font-family: monospace; }
